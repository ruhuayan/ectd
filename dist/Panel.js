    ...
    Jstree.prototype = {
        ...
        setSelectList: function(json){   //console.log(json)
            for(var i in json){
                var node = json[i];
                if(node['type']==="file") {
            ...
            $(document).on('dnd_move.vakata', function (e, data) {                    //console.log(data.element.text.indexOf(".pdf"))
                ...
                if(t.parents('div.pdf-editor').length && pdfEditor.panel.attr('data-loaded')=='false')
                    pdfEditor.setEditCount()
                        .openPanel(fileName, fileId, function(thisPanel){
                            // pdfEditor.replaceEdits(_this);
                            pdfEditor.loadEdits(fileId).then(
                                res=>{
                                    if(res && res.action){          // console.log(res.action)
                                        thisPanel.edits = JSON.parse(res.action);
                                        pdfEditor.replaceEdits(thisPanel);
                                    }
                                }
                            );
    ...
    PdfPanel.prototype = {
        ...
        openPanel: function(file, fileId, callback){
            ...
            if(_this.id =="pdf-frame"){
                ...
                // this.loadEdits(fileId).then(
                //     res=>{
                //         if(res && res.action){           console.log(res.action)
                //             _this.edits = res.action;
                //             this.replaceEdits(_this);
                //         }
                //     }
                // );
            }
            ...
            xhr.onload = function() {
                if (this.status === 200) {
                    ...
                    loadingTask.promise.then(function(pdf){
                        ...
                        _this.panel.find('div.load-process').fadeOut(1000);          //console.log('file load done')
    ...
    PdfEditor.prototype = {
        ...
        getEditList: function(){
            var _this = this, edits = {'links':[], 'texts': []}, pageWrap = this.pageWrap;
            this.panel.find(".target-editable").each(function(index, value){       //console.log('inside at function - target', new Big(1, 5));
                ...
                p.id && edits.links.push(p);                    //console.log(c); to JSON.stringify(p) before sending to server
            });
        ...
        replaceEdits: function(_this){               //console.log(_this)
            if(!_this.edits) return false;
            var editZone = _this.fileZone;
            var linkEdits = _this.edits["links"];           console.log(linkEdits)
            var textEdits = _this.edits["texts"];
